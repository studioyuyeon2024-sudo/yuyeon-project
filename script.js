...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('matching-form').onsubmit = async (e) => {
        ...
        try {
            ...
            if (adminDoc.exists() && adminDoc.data().is_open) return alert("접수 마감되었습니다.");
        } catch (err) { console.error(err); }

        if (!document.getElementById('privacy-check').checked) return alert("동의 필수입니다.");
        if (!userIdInput.value) return alert("번호 선택 필수입니다.");
    ...
    document.getElementById('check-result-btn').onclick = async () => {
        const adminDoc = await getDoc(doc(db, "settings", "matching_status"));
        if (!adminDoc.exists() || !adminDoc.data().is_open) return alert("결과 공개 전입니다.");
