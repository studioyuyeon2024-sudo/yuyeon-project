...
window.checkAdmin = async function() {
    ...
    try { await signInWithEmailAndPassword(auth, email, pw); } catch (e) { alert("실패"); }
};

onAuthStateChanged(auth, (user) => { if (user) { document.getElementById('auth-section').style.display='none'; document.getElementById('report-section').style.display='block'; updateDashboard(); } });
...
async function updateDashboard() {
    try {
        ...
        currentData.sort((a,b) => a.myId - b.myId).forEach(p => {
            listBody.innerHTML += `<tr><td>${p.myId}</td><td>${p.gender==='male'?'남':'여'}</td><td>${p.realName}</td><td>${p.phone}</td><td>${p.pickId1||'-'},${p.pickId2||'-'}</td><td style="color:#FF5252; font-weight:bold;">${voteMap[p.myId]||0}표</td><td class="review-cell">${p.review || '-'}</td></tr>`;
        });
        ...
        males.forEach(m => {
            females.forEach(f => {
                if ([m.pickId1, m.pickId2].includes(f.myId) && [f.pickId1, f.pickId2].includes(m.myId)) {
                    ...
                    miniMatchArea.innerHTML += `<div style='padding:8px; background:#f0f4ff; border-radius:10px; margin-bottom:5px;'><b>${m.myId}(${m.realName}/${m.phone})</b> ❤️ <b>${f.myId}(${f.realName}/${f.phone})</b></div>`;
                }
            });
        });
...
function verify() { return prompt("2차 비번") === EXPORT_PASSWORD; }
document.getElementById('open-btn').onclick = async () => { await setDoc(doc(db, "settings", "matching_status"), { is_open: true }); alert("공개됨"); };
...
document.getElementById('delete-btn').onclick = async () => {
    const batch = prompt("기수 이름");
    if (!batch || !verify()) return;
    try {
        const promises = currentData.map(p => setDoc(doc(db, "archive", `${batch}_${p.phone}`), { ...p, batchName: batch, archivedAt: new Date() }));
        await Promise.all(promises);
        const snap = await getDocs(collection(db, "participants"));
        await Promise.all(snap.docs.map(d => deleteDoc(doc(db, "participants", d.id))));
        alert("성공"); location.reload();
