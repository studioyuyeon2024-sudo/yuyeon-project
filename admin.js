...
const EXPORT_PASSWORD = "yuyeon_secure_777"; // 우철님만 아는 2차 비밀번호
...
document.getElementById('open-btn').onclick = async () => { if(confirm("결과를 공개할까요?")) { await setDoc(doc(db, "settings", "matching_status"), { is_open: true }); alert("공개 완료"); } };
document.getElementById('download-btn').onclick = () => {
    ...
    link.download = `유연_현재명단_${new Date().getTime()}.csv`; link.click();
};
...
document.getElementById('download-archive-btn').onclick = async () => {
    ...
    } catch(e) { alert("백업 다운로드 오류: " + e.message); }
};

document.getElementById('delete-btn').onclick = async () => {
    const batch = prompt("백업할 기수 이름을 입력하세요 (예: 유연 1기)");
    if (!batch || !confirm(`${batch}로 백업 후 현재 데이터를 삭제하시겠습니까?`)) return;
    try {
...
document.getElementById('close-btn').onclick = async () => { if(confirm("공개를 차단할까요?")) { await setDoc(doc(db, "settings", "matching_status"), { is_open: false }); alert("차단 완료"); } };
